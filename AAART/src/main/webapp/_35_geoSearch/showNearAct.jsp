...
<jsp:include page="/fragment/top.jsp" />
	...
	<c:forEach var="nearAct" items="${nearActList}">
		<c:if test="${nearAct.distance != '0'}">
			...
			<c:set var="addr" value="${nearAct.city.concat(nearAct.district).concat(nearAct.village).concat(nearAct.address)}"/>
			<c:if test="${counter < 25}">
				<iframe width="100%" height="300" frameborder="0" src="https://www.google.com/maps?q=${addr}&output=embed"></iframe>
			</c:if>
			<br>
		</c:if>
	</c:forEach>
